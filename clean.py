...
def clean_data(df, logger):
    ...
    return df, total_cells, filled_values, rows_before, rows_removed
...
def generate_report(total_cells, filled_values, rows_before, rows_removed, logger):
    try:
...
def main():
    ...
    df = load_data(data_file_path, logger)
    logger.info(f"First few rows of the data:\n{df.head()}")

    df, total_cells, filled_values, rows_before, rows_removed = clean_data(df, logger)
    df = engineer_features(df)
    logger.info('Feature engineering completed.')

    generate_report(total_cells, filled_values, rows_before, rows_removed, logger)

    logger.info(f'Filled {filled_values} missing values ({(filled_values / total_cells) * 100:.2f}% of total data).')
    logger.info(f'Removed {rows_removed} rows ({(rows_removed / rows_before) * 100:.2f}% of total rows).')
